<template>
  <div>
    <h1>Bookings</h1>
    <router-link tag="b-button" :to="{ name: 'createBooking' }"
      >Add a booking</router-link
    >
    <b-table striped :items="bookings" :fields="fields">
      <template v-slot:cell(guestLastName)="row">
        <router-link
          :to="{ name: 'booking', params: { bookingId: row.item._id } }"
          >{{ row.item.guestLastName }}</router-link
        >
      </template>
      <template v-slot:cell(edit)="row">
        <router-link
          tag="b-button"
          :to="{ name: 'editBooking', params: { bookingId: row.item._id } }"
          >Edit</router-link
        >
      </template>
