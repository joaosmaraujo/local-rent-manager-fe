<template>
    <div>
        <h1>Houses</h1>
        <router-link tag="b-button" :to="{ name: 'createHouse' }">Add a house</router-link>
        <b-table striped :items="houses" :fields="fields">
            <template v-slot:cell(address)="row">
                <router-link :to="{ name: 'house', params: { houseId: row.item._id } }">{{
                    row.item.address
                }}</router-link>
            </template>
            <template v-slot:cell(edit)="row">
                <router-link tag="b-button" :to="{ name: 'editHouse', params: { houseId: row.item._id } }"
                    >Edit</router-link
                >
            </template>
