<template>
  <div>
    <h1>Works</h1>
    <router-link tag="b-button" :to="{ name: 'createWork' }"
      >Add a work</router-link
    >
    <b-table striped :items="works" :fields="fields">
      <template v-slot:cell(name)="row">
        <router-link :to="{ name: 'work', params: { workId: row.item._id } }">{{
          row.item.name
        }}</router-link>
      </template>
      <template v-slot:cell(edit)="row">
        <router-link
          tag="b-button"
          :to="{ name: 'editWork', params: { workId: row.item._id } }"
          >Edit</router-link
        >
      </template>
